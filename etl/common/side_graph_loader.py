...
RAW_DIR = ETL_DIR / "raw"
RUNTIME_DIR = ROOT_DIR / "runtime" / "etl" / "side-graph-load"
...
PREVIEW_GEOJSON = ETL_DIR / "subway_elevator_preview.geojson"
PREVIEW_HTML = ETL_DIR / "subway_elevator_preview.html"
...
def clip_to_intersection_free_parts(parts: list[tuple[tuple[float, float], ...]], intersection_union: Any) -> list[tuple[tuple[float, float], ...]]:
    ...
    for part in parts:
        ...
        if intersection_union is not None and not intersection_union.is_empty:
            geom = geom.difference(intersection_union)
...
def build_base_lines(
    ...
    intersection_union = build_intersection_union(cluster_centers, intersection_roots, radii)
    ...
    trimmed_lines = trim_side_line_tails(base_lines, intersection_union)
...
def trim_side_line_tails(base_lines: list[BaseLine], intersection_union: Any) -> list[BaseLine]:
    if intersection_union is None or intersection_union.is_empty:
        return base_lines
    ...
    for line in base_lines:
        ...
        start_near_intersection = Point(line.coords[0]).distance(intersection_union) <= INTERSECTION_NEAR_M
        end_near_intersection = Point(line.coords[-1]).distance(intersection_union) <= INTERSECTION_NEAR_M
    ...
    *,
    ...
    for _, source_line, endpoint_index, source_point, outward in endpoint_candidates:
        ...
        if cluster_centers is not None and intersection_roots and intersection_radii:
            ...
            for root in intersection_roots:
                if point_distance_meter(source_point, cluster_centers[root]) <= max(intersection_radii.get(root, OFFSET_FLOOR_M), JUNCTION_POCKET_RADIUS_M):
        ...
        bridge_key = tuple(sorted((rounded_point(source_point), rounded_point(attach_point))))
    ...
    return connectors, events, {"gapBridgeCount": len(connectors)}
    ...
    *,
    ...
    for key, group in assignments_by_key.items():
        ...
        for assignment in sorted(group, key=lambda item: point_distance_meter(item.point, root_point)):
            ...
            rounded_source = (round(assignment.point[0], 3), round(assignment.point[1], 3))
            ...
            for target_line in group_lines:
                ...
                if distance_meter <= MIN_SPLIT_GAP_M or distance_meter > max_distance:
                    continue
                target_rounded = (round(attach_point[0], 3), round(attach_point[1], 3))
            ...
            occupied_points.add((round(attach_point[0], 3), round(attach_point[1], 3)))

    return connectors, events, {"sameSideCornerBridgeCount": len(connectors)}
    ...
    *,
    ...
    for (root, sector_index), group in assignments_by_key.items():
        ...
        for source in sorted(left_assignments + right_assignments, key=lambda item: point_distance_meter(item.point, root_point)):
            ...
            rounded_source = (round(source.point[0], 3), round(source.point[1], 3))
            if source_key in used_endpoints or rounded_source in occupied_points:
                continue
    ...
    return connectors, events, {"crossSideCornerBridgeCount": len(connectors)}
...
def cleanup_junction_pockets(
    ...
    for segment_type, coords in projected_segments:
        ...
        start_pocket = next((center for center in canonical_pockets if point_distance_meter(start, center) <= JUNCTION_POCKET_RADIUS_M), None)
        end_pocket = next((center for center in canonical_pockets if point_distance_meter(end, center) <= JUNCTION_POCKET_RADIUS_M), None)
...
def build_side_graph(
    *,
    ...
    split_segments = split_base_lines(cleaned_base_lines, all_events)
    connector_segments = [
    ...
    consolidated_segments, junction_stats = consolidate_junction_candidates(resolved_segments)
