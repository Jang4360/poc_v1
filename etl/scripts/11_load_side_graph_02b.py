...
from etl.common import side_graph_loader_02b, subway_elevator_preview


def main() -> int:
    parser = argparse.ArgumentParser(description="Build 02B side graph snapshots and Kakao preview.")
    ...
    parser.add_argument("--load-db", action="store_true", help="Load generated 02B side graph into PostGIS.")
    parser.add_argument("--output-html", type=Path, default=side_graph_loader_02b.PREVIEW_HTML)
    parser.add_argument("--output-geojson", type=Path, default=side_graph_loader_02b.PREVIEW_GEOJSON)
    args = parser.parse_args()

    payload, report, audit = side_graph_loader_02b.generate_preview_dataset(
    ...
    if args.load_db:
        nodes, segments, _ = side_graph_loader_02b.build_side_graph(
            center_lat=args.center_lat,
            center_lon=args.center_lon,
            radius_m=args.radius_meter,
        )
        result["dbLoad"] = side_graph_loader_02b.load_db(nodes, segments)

    print("side-graph-02b-load: ok")
