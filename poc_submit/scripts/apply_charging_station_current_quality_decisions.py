...
ARCHIVE_DIR = POC_ROOT / "archive" / "20260427_charging_station_current_quality_before"

PLAN_CSV = REPORT_DIR / "charging_station_current_quality_plan.csv"
OUT_RENAMED = REPORT_DIR / "charging_station_current_quality_decisions_renamed.csv"
OUT_REMOVED = REPORT_DIR / "charging_station_current_quality_decisions_removed.csv"
OUT_MANUAL = REPORT_DIR / "charging_station_current_quality_remaining_manual.csv"
OUT_SUMMARY = REPORT_DIR / "charging_station_current_quality_decisions_summary.json"
...
def main() -> None:
    ...
    decisions = load_plan()
    rows = read_csv(base.ADOPTED_ALL)
    fieldnames = list(rows[0].keys())
    before_charging = [row for row in rows if row["dbCategory"] == "CHARGING_STATION"]
    ...
    for row in rows:
        ...
        if row["dbCategory"] != "CHARGING_STATION" or not decision:
        ...
        if new_name and new_name != old_name:
            row["name"] = new_name
            row["reviewFlags"] = append_pipe(row.get("reviewFlags", ""), "charging_station_current_quality_renamed")
            row["reviewReasons"] = append_pipe(row.get("reviewReasons", ""), f"전동보장구 충전소 표시명 보정: {old_name} -> {new_name}")
            renamed.append(
                {
                    "sourceKey": row["sourceKey"],
                    "placeIdBefore": row["placeId"],
    ...
    write_csv(base.ADOPTED_ALL, kept_rows, fieldnames)
    ...
    after_charging = [row for row in kept_rows if row["dbCategory"] == "CHARGING_STATION"]
    summary = {
        "before": {"places": len(rows), "chargingStation": len(before_charging)},
        "after": {"places": len(kept_rows), "chargingStation": len(after_charging)},
        "renamed": len(renamed),
        "removed": len(removed),
