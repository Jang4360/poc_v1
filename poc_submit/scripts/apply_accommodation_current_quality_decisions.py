...
ARCHIVE_DIR = POC_ROOT / "archive" / "20260427_accommodation_current_quality_before"

ADOPTED_ALL = DATA_ADOPTED / "adopted_places_with_accessibility.csv"
PLAN_CSV = REPORT_DIR / "accommodation_current_quality_plan.csv"
OUT_RENAMED = REPORT_DIR / "accommodation_current_quality_decisions_renamed.csv"
OUT_REMOVED = REPORT_DIR / "accommodation_current_quality_decisions_removed.csv"
OUT_MANUAL = REPORT_DIR / "accommodation_current_quality_remaining_manual.csv"
OUT_SUMMARY = REPORT_DIR / "accommodation_current_quality_decisions_summary.json"
...
def main() -> None:
    ...
    before_accommodation = [row for row in rows if row["dbCategory"] == "ACCOMMODATION"]
    ...
    for row in rows:
        ...
        if row["dbCategory"] != "ACCOMMODATION" or not decision:
        ...
        if new_name and new_name != old_name:
            row["name"] = new_name
            row["reviewFlags"] = append_pipe(row.get("reviewFlags", ""), "accommodation_current_quality_renamed")
            row["reviewReasons"] = append_pipe(row.get("reviewReasons", ""), f"숙박 표시명 보정: {old_name} -> {new_name}")
            renamed.append(
                {
                    "sourceKey": row["sourceKey"],
                    "placeIdBefore": row["placeId"],
    ...
    after_accommodation = [row for row in kept_rows if row["dbCategory"] == "ACCOMMODATION"]
    summary = {
        "before": {"places": len(rows), "accommodation": len(before_accommodation)},
        "after": {"places": len(kept_rows), "accommodation": len(after_accommodation)},
        "renamed": len(renamed),
        "removed": len(removed),
