...
ARCHIVE_DIR = POC_ROOT / "archive" / "20260427_restaurant_current_quality_before"
...
PLAN_CSV = REPORT_DIR / "restaurant_current_quality_plan.csv"
OUT_RENAMED = REPORT_DIR / "restaurant_current_quality_decisions_renamed.csv"
OUT_REMOVED = REPORT_DIR / "restaurant_current_quality_decisions_removed.csv"
OUT_SUMMARY = REPORT_DIR / "restaurant_current_quality_decisions_summary.json"
...
def load_plan() -> dict[str, dict[str, str]]:
    return {
        ...
        if row["recommendedAction"] in {"RENAME_CANDIDATE", "REMOVE_OR_MANUAL"}
    }
...
def update_adopted_all(plan: dict[str, dict[str, str]]) -> tuple[list[dict[str, str]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in rows:
        ...
        if row["dbCategory"] != "RESTAURANT" or not decision:
            kept.append(row)
            continue

        if decision["recommendedAction"] == "REMOVE_OR_MANUAL":
            removed.append(
                {
        ...
        if new_name and new_name != old_name:
            row["name"] = new_name
            row["reviewFlags"] = append_pipe(row.get("reviewFlags", ""), "restaurant_current_quality_renamed")
            row["reviewReasons"] = append_pipe(row.get("reviewReasons", ""), f"음식·카페 표시명 보정: {old_name} -> {new_name}")
            renamed.append(
                {
                    "sourceKey": row["sourceKey"],
                    "placeIdBefore": row["placeId"],
...
def update_facilities_js(kept_rows: list[dict[str, str]]) -> None:
    ...
    for feature in geojson["features"]:
        ...
        if adopted["dbCategory"] == "RESTAURANT":
            flags = props.get("reviewFlags", [])
            if "restaurant_current_quality_renamed" in split_pipe(adopted.get("reviewFlags", "")):
                if isinstance(flags, list) and "restaurant_current_quality_renamed" not in flags:
                    flags.append("restaurant_current_quality_renamed")
...
def main() -> None:
    ...
    before_restaurants = [row for row in before_rows if row["dbCategory"] == "RESTAURANT"]
    ...
    write_csv(
        OUT_RENAMED,
        renamed,
        ["sourceKey", "placeIdBefore", "oldName", "newName", "districtGu", "address", "suggestionSource", "suggestionCategory", "reason"],
    )
    write_csv(
        OUT_REMOVED,
        removed,
        ["sourceKey", "placeIdBefore", "name", "districtGu", "address", "rawFacilityType", "removeReason"],
    )

    after_restaurants = [row for row in kept_rows if row["dbCategory"] == "RESTAURANT"]
    summary = {
        "before": {"places": len(before_rows), "restaurants": len(before_restaurants)},
        "after": {"places": len(kept_rows), "restaurants": len(after_restaurants)},
        "renamed": len(renamed),
        "removed": len(removed),
        "outputs": {"renamed": str(OUT_RENAMED), "removed": str(OUT_REMOVED)},
    }
